...
import { useFetchDeveloperBookingDetails, DeveloperBookingDetails } from '../../hooks/useFetchDeveloperBookingDetails';

const colors = themeColors.dark; // Or themeColors.dark based on developer theme

type DeveloperBookingDetailsScreenRouteProp = RouteProp<DeveloperBookingsStackParamList, 'DeveloperBookingDetails'>;
...
const DeveloperBookingDetailsScreen: React.FC = () => {
  ...
  const { data: booking, isLoading, isError, error } = useFetchDeveloperBookingDetails(bookingId);
  ...
  if (isError || !booking) {
    return <Text style={styles.errorText}>{error?.message || 'Booking not found or error loading details.'}</Text>;
  }
  ...
  const bookingStartTime = new Date(booking.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
  const bookingEndTime = new Date(booking.end_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      <View style={styles.card}>
        ...
        <View style={styles.detailRow}>
          ...
          <Text style={styles.detailValue}>{`${bookingStartTime} - ${bookingEndTime}`}</Text>
        </View>
        <View style={styles.detailRow}>
          <Ionicons name="checkmark-circle-outline" size={20} color={booking.status === 'confirmed' ? colors.success : colors.warning} style={styles.icon} />
          <Text style={styles.detailLabel}>Status:</Text>
          <Text style={[styles.detailValue, { color: booking.status === 'confirmed' ? colors.success : colors.warning }]}>
            {bookingStatus}
          </Text>
        </View>
        {booking.notes && (
        ...
        <View style={styles.detailRow}>
          ...
          <Text style={[styles.detailValue, styles.bookingIdText]}>{booking.id}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  clientName: { // Changed from developerName
    fontSize: 22,
    fontWeight: 'bold',
    color: colors.text,
    textAlign: 'center',
