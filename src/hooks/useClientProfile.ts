...
import { ClientProfile, FetchedClientProfile } from "../types";

// Fetches combined user and client profile data
async function fetchClientProfile(
  userId: string,
): Promise<FetchedClientProfile | null> {
  if (!userId) {
    return null;
  }
  ...
  try {
    ...
    const { data: clientData, error: clientError, status: clientStatus } =
      await supabase
    ...
    const combinedProfile: FetchedClientProfile = {
      ...
      ...(clientData ||
        { id: userId, client_name: "", created_at: new Date().toISOString() }), // Provide defaults if clientData is null
