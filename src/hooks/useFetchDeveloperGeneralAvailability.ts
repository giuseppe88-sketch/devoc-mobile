...
async function fetchGeneralAvailabilityByProfileId(developerProfileId: string | undefined): Promise<Availability[]> {
  if (!developerProfileId) {
    // console.warn('[fetchGeneralAvailabilityByProfileId] developerProfileId is missing, returning empty array.'); // Commented out
    return [];
  }
  ...
  if (error) {
    // console.error(`Error fetching general availability for profile ${developerProfileId}:`, error.message); // Commented out
