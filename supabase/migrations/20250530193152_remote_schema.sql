...
CREATE OR REPLACE FUNCTION public.create_booking_and_update_slot(p_client_id uuid, p_developer_id uuid, p_slot_id uuid)
 RETURNS TABLE(booking_id uuid, booked_start_time timestamp with time zone, booked_end_time timestamp with time zone, booking_status text) -- Changed to WITH TIME ZONE
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
  ...
  v_calculated_start_timestamp TIMESTAMP WITH TIME ZONE; -- Changed to WITH TIME ZONE
  v_calculated_end_timestamp TIMESTAMP WITH TIME ZONE;   -- Changed to WITH TIME ZONE
...
BEGIN
  ...
  IF NOT FOUND THEN
    -- CORRECTED BLOCK: Check against public.availabilities
  ...
  v_calculated_start_timestamp := (v_target_date + v_slot_start_time)::TIMESTAMP WITH TIME ZONE;
  v_calculated_end_timestamp := (v_target_date + v_slot_end_time)::TIMESTAMP WITH TIME ZONE;
